...
public class MockApiController {
     ...
     */
    private ResponseEntity<String> execute(HttpServletRequest request, String mockPath) {
    ...
    public ResponseEntity<String> executeGet(HttpServletRequest request, @RequestHeader(name = "mock-path") String mockPath) {
        return execute(request, mockPath);
    }

    @Operation(summary = "Mock a MSP or a MaaS", description = "Use 'mock-path' header to respond with a return code and an optional body. A value like 'case/200/ok.json' will respond a HTTP 200 with the content of case/200/OK.json file in the body.")
    @PostMapping(value = MOCK_OPERATION_NAME, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> executePost(HttpServletRequest request, @RequestHeader(name = "mock-path") String mockPath) {
        return execute(request, mockPath);
    }

    @Operation(summary = "Mock a MSP or a MaaS", description = "Use 'mock-path' header to respond with a return code and an optional body. A value like 'case/200/ok.json' will respond a HTTP 200 with the content of case/200/OK.json file in the body.")
    @PutMapping(value = MOCK_OPERATION_NAME, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> executePut(HttpServletRequest request, @RequestHeader(name = "mock-path") String mockPath) {
        return execute(request, mockPath);
    }

    @Operation(summary = "Mock a MSP or a MaaS", description = "Use 'mock-path' header to respond with a return code and an optional body. A value like 'case/200/ok.json' will respond a HTTP 200 with the content of case/200/OK.json file in the body.")
    @PatchMapping(value = MOCK_OPERATION_NAME, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> executePatch(HttpServletRequest request, @RequestHeader(name = "mock-path") String mockPath) {
        return execute(request, mockPath);
    }

    @Operation(summary = "Mock a MSP or a MaaS", description = "Use 'mock-path' header to respond with a return code and an optional body. A value like 'case/200/ok.json' will respond a HTTP 200 with the content of case/200/OK.json file in the body.")
    @DeleteMapping(value = MOCK_OPERATION_NAME, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<String> executeDelete(HttpServletRequest request, @RequestHeader(name = "mock-path") String mockPath) {
        return execute(request, mockPath);
    }
