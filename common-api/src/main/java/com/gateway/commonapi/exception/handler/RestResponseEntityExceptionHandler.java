...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException
        ...
        if (exception.getCause() != null) {
            if (exception.getCause().getCause() != null && StringUtils.isNotBlank(exception.getCause().getCause().getMessage())) {
                ...
                errorBody.setDescription(UNEXPECTED_FORMAT);

            } else if (StringUtils.isNotBlank(exception.getCause().getMessage())) {

                if (ExceptionUtils.getRootException(exception, InvalidFormatException.class) != null && Objects.requireNonNull(exception.getMessage()).contains(UUID_PARSE_ERROR)) {
                    errorBody.setDescription(INVALID_FORMAT_UUID + " " + ((InvalidFormatException) exception.getCause()).getValue());
                    errorBody.setErrorCode(INVALID_PARTNER_UUID_CODE);
                    errorBody.setLabel(INVALID_FORMAT_UUID);
                } else {
            ...
            // in the other cases, we set the description and the label generated by the method from error messages
    ...
    private ResponseEntity<Object> getObjectResponseEntity(HttpHeaders headers, HttpStatus status, WebRequest
        ...
        if (status.equals(HttpStatus.BAD_REQUEST)) {
            return handleExceptionInternal(new BadRequestException(errorObject), errorObject, headers, status, request);
        } else {
