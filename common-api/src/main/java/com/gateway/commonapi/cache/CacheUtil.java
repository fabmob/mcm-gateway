...
public class CacheUtil<CacheStoredType extends String, DtoGenericType extends Object> {
     ...
     */
    @Value("${spring.redis.keys.prefix:default}")
     ...
     */
    public void putValue(String key, CacheStoredType value) {
        String cacheKey = this.getKeyPrefixEnv() + key;
        // if element is already a string store it other convert it into string
     ...
     */
    public void addGeoMetadata(String key, double latitude, double longitude, CacheStoredType element) {
        RedisGeoCommands.GeoLocation<CacheStoredType> geoLocation = null;
        String geoCacheKey = this.getKeyPrefixEnv() + GEO_PREFIX + key;
    ...
    private List<GeoResult<RedisGeoCommands.GeoLocation<CacheStoredType>>> searchByDistance(String geoKey,
                                                                                            ...
                                                                                            int radius, RedisGeoCommands.DistanceUnit radiusDistanceUnit) {
     ...
     *
     * @param mspId        Id of the msp in string format to search in the cache associated with it
     * @param serviceName  Name of the service used for the cache key. eg : aroundMe, availableAssets, ...
     * @param objectType   Object type of the service. eg: Assets, Parking, ... for aroundMe service
     ...
     * @return
     */
    public List<Pair<Distance, DtoGenericType>> searchAndRetrieveByDistance(String mspId, String serviceName, String objectType,
                                                                            double latitude, double longitude,
                                                                            int radius, RedisGeoCommands.DistanceUnit distanceUnit, Class<DtoGenericType> dtoClass) {
        ...
        String geoKey = this.getKeyPrefixEnv() + GEO_PREFIX;

        // get geo elements from cache regarding a geoSearch. Element as result have has name the identifier of the object and also have geo metadatas as calculated distance
        List<GeoResult<RedisGeoCommands.GeoLocation<CacheStoredType>>> foundGeoElement = this.searchByDistance(geoKey + mspId + "-" + serviceName, latitude, longitude, radius, distanceUnit);
        ...
        String prefixDtoElement = this.getKeyPrefixEnv() + mspId + "-" + serviceName + "-" + objectType + "_";

        // get all elements in cache in a map as DTO class as value
        Map<String, DtoGenericType> convertedElements = this.getValues(prefixDtoElement, elementsToRetrieve, dtoClass);
    ...
    public DtoGenericType getValue(String key, Class<DtoGenericType> className) {
        ...
        try {
            res = new ObjectMapper().readValue(String.valueOf(valueOperations.get(key)), className);
        } catch (IOException e) {
    ...
    public Set<String> getKeysByPrefix(String prefix) {
        return getKeysPattern(this.keyPrefixEnv + prefix + "*");
    }
    ...
    public void clearCache(String keyPattern, boolean ignoreEnvPrefix) {
        ...
        Set<String> keysToDelete = this.redisTemplate.keys(ignoreEnvPrefix ? keyPattern : this.keyPrefixEnv + keyPattern);
        log.info("Pattern {} matching {} results, will be deleted from cache", this.keyPrefixEnv + keyPattern, keysToDelete.size());
